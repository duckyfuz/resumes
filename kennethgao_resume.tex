...
\begin{tabular*}{\textwidth}{l@{\extracolsep{\fill}}r}
  \textbf{\large{Kenneth Gao}} $|$ {\href{mailto:kenneth.gao@u.nus.edu}{kenneth.gao@u.nus.edu}} $|$ {\href{www.kenf.dev}{\underline{www.kenf.dev}}} & \textbf{Availability: }Jan. 2026 onwards \\
...
\section{Education}
  \resumeSubHeadingListStart
    ...
    \resumeDescription{B.Eng Computer Engineering (E-Scholars Programme), IEEE-Eta Kappa Nu (HKN) $|$ 5.0 GPA}
...
\section{Experience}
  \resumeSubHeadingListStart
    ...
    \resumeSubheading{\textbf{Software Engineering Intern @ \textit{Open Government Products (OGP)}}}{Sep. 2025 - Present}
    \resumeDescription{OGP builds technology for the public good, solving problems for citizens and public officers}
    \resumeItemListStart
      \resumeItem{Designed a fault-tolerant archival pipeline for offloading bloated tables using AWS EventBridge and ECS, resulting in a 40\% increase in query speed by reducing database contentions and deadlocks}
      \resumeItem{Integrated structured state tracking to ensure complete observability and reliable retries of archival jobs}
      \resumeItem{Enhanced observability with Datadog by implementing structured logging and APM tracing for e2e traceability}
      % \resumeItem{Engineered a new jobs architecture and archival pipeline using AWS EventBridge and ECS, improving system reliability and reducing database contentions to increase query performance by 40\%}
      % \resumeItem{Developed a low-memory, fault-tolerant report generation service using ECS to stream data between S3 buckets, enabling efficient aggregation and delivery of large-scale data exports}
      \resumeItem{Leveraged Localstack for development and integration tests, eliminating the need for complex and flaky mocks}
      % \resumeItem{Improved developer experience by integrating Localstack for local AWS service testing and optimizing CI/CD pipelines through Docker layer caching}
      % \resumeItem{Extended application functionality by implementing bulk operation endpoints, team transfer features, and UI improvements for reports and data management}
    \resumeItemListEnd
    ...
    \resumeItemListStart
      ...
      \resumeItem{Explored rate limiter implementations to ensure system stability via managing API request rates}
    ...
    \resumeItemListStart
      ...
      \resumeItem{Optimized daily pg\_cron jobs to ingest thousands of session results for identifying students' areas for improvement}
...
\section{Projects / Extracurriculars}
  \resumeSubHeadingListStart
    ...
    \resumeSubheading{\textbf{Proxmox VE Homelab} $|$ repurposed laptops running LXC containers and VMs}{Feb. 2024 - Present}
    \resumeItemListStart
      \resumeItem{Implemented HA for 2-node Proxmox cluster using QDevice for quorum and ZFS replication for data redundancy}
      \resumeItem{Enhanced security posture by facilitiating p2p connections with Tailscale as a Zero Trust Network (ZTN) solution}
    ...
    \resumeItemListStart
      ...
      \resumeItem{Adopted by 50+ users for sending files to public printers, eliminating the need for insecure public machine logins}
    \resumeItemListEnd
